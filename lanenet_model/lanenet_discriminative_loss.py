...
def discriminative_loss_single(
    ...
    # 统计实例个数num_instances,并获得掩模unique_id
    unique_labels, unique_id, counts = tf.unique_with_counts(correct_label)
    counts = tf.cast(counts, tf.float32)
    num_instances = tf.size(unique_labels)

    # 计算pixel embedding均值向量
    ## 计算每一个feature map中每一种label的均值
    segmented_sum = tf.unsorted_segment_sum(
        reshaped_pred, unique_id, num_instances)
    print('semented_sum_shape = %s' % segmented_sum.get_shape().as_list())
    mu = tf.div(segmented_sum, tf.reshape(counts, (-1, 1)))
    ## mu = tf.reshape(tensor=mu, shape=[1, 3])
    print('mu_shape = %s' % mu.get_shape().as_list())
    print('unique_id_shape = %s' % unique_id.get_shape().as_list())

    mu_expand = tf.gather(mu, unique_id)
    print('mu_expand_shape = %s' % mu_expand.get_shape().as_list())
    ...
    l_var = tf.unsorted_segment_sum(distance, unique_id, num_instances) ## l_var的内层求和运算
    l_var = tf.div(l_var, counts)
    l_var = tf.reduce_sum(l_var)                                        ## l_var的外层求和运算
    l_var = tf.divide(l_var, tf.cast(num_instances, tf.float32))

    # 计算公式的loss(dist)
    mu_interleaved_rep = tf.tile(mu, [num_instances, 1])
    print("mu_interleaved_rep = %s" % mu_interleaved_rep.get_shape().as_list())
    mu_band_rep = tf.tile(mu, [1, num_instances])
    print("mu_band_rep = %s" % mu_band_rep.get_shape().as_list())
    ...
    mu_norm = tf.subtract(delta_d, mu_norm)
